...
**Version**: 1.18.0 | **Generated by**: autoharness
...
**Efficient wave-set computation (snapshot recipe).** Computing `ready_k` (step 1) and the
requery (step 6) from **one dependency lookup per task, repeated at every wave**, costs up to
`count(T) × waves` calls — as many as 42 × 18 = 756 on this release unit's own schedule — for
...
reused, unchanged, through that same wave's step 6 requery. The snapshot is never carried over to
...
* **When `.autoharness/backlog-registry.yaml` declares `features.sql_query: true`** (the `query`
  ...
  d.item_id = i.id WHERE i.parent_id = '{release_unit}' AND i.status != 'archived'`. One result set
  carries both every task's current status and every dependency edge; `ready_k` is every `queued`
  id all of whose `depends_on` rows resolve to a `done` status in that same result set.
* **Otherwise, fall back to the `list_tasks` operation** (`backlogit_list_items` / `backlogit list
  --type task --json`, scoped to the release unit), which embeds each item's `dependencies` array
  inline (`models.Artifact.Dependencies`). This is CLI/MCP **parity**, not a degraded substitute: it
  is the same one-call-per-wave snapshot, computing `ready_k` from the embedded edges instead of a
  join, for a registry that has no `query` operation.
* **Fail closed on an unreliable snapshot.** If the query or list call errors, returns
  non-parseable output, or returns a task count that does not match the release unit's known task
  count, do not use it to admit or requery a wave — halt with `WAVE_SNAPSHOT_UNRELIABLE` (P-002.2)
  and report the mismatch. Do not substitute a per-task fallback, do not reuse a prior wave's
  snapshot, and do not proceed on a partial result. A degraded query *tool* (as opposed to an
  unsupported *operation*) is a P-012 `TOOL_DEGRADED`/`TOOL_UNAVAILABLE` condition and falls back to
  the `list_tasks` form above rather than retrying the same call.
...
`WAVE_CYCLE_DETECTED`, `WAVE_BUDGET_EXCEEDED`, or `WAVE_SNAPSHOT_UNRELIABLE`) through P-005
telemetry with the deterministic blocked report, and return the release unit to the operator or to
Stage for a dependency-graph amendment. Do not claim a task whose dependencies are unfinished, do
not scaffold outside the current wave, and do not exempt a task to make a wave admit it.
