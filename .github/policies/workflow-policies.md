...
**Applies when**: `Shipments group related work items into a single release unit. Each shipment tracks its items through queued -> active -> done -> shipped lifecycle.` is true and `backlogit` is `backlogit`.
