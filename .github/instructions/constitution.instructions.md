...
No ``unsafe`` package usage without explicit justification in a code comment and review approval.. ``golangci-lint run`` must pass with zero warnings before any commit. ``go vet ./...`` must also pass..
...
v2.13.2. Windows and Linux build-tag surfaces are verified separately.
The exception expires at the declared unique terminal boundary, where the
canonical terminal runner must report zero warnings on both supported GOOS surfaces before
...
**Current sync impact (1.1.0)**: Principle I now admits only the P-002.6
baseline-convergence timing exception defined above. The rationale is to make
removal of a known inherited lint baseline executable without admitting new
debt. `AGENTS.md`, Ship, build-feature, workflow policy, and the executable
simulation carry the same bounded rule; strict-mode and terminal zero-warning
gates are unchanged.

**Version**: 1.1.0 | **Ratified**: 2026-05-11 | **Generated by**: autoharness
