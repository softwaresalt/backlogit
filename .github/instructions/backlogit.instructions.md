...
6. `list_checkpoints` is read-only: it never physically quarantines a malformed checkpoint as a side effect of listing. A malformed entry is surfaced with `needs_quarantine: true` and a `remediation_command`; use `abandon_checkpoint` for a valid, parseable checkpoint or `quarantine_checkpoint` for a malformed one -- the two verbs are disjoint by design and each refuses to operate on the other's target class. Both require an explicit, non-empty `operator` (never inferred) and a non-empty `reason`. See `docs/design-docs/checkpoint-administrative-disposition.md`.
