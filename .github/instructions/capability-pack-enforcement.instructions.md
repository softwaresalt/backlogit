...
This workspace enables only the `agent-engram` retrieval-enforced pack (the
`graphtor-docs` pack is not installed here); the route and deferral blocks
above are scoped to exactly that enabled set. If `graphtor-docs` is enabled in
a future tune, both blocks must be re-expanded to include its route and
deferral rows together, in the same pass.
...
Generated by autoharness | Template: capability-pack-enforcement.instructions.md.tmpl
