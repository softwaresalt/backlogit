...
* **Halted-archival third branch (143-F, workspace-specific)**: when the cascade
  path was used and `backlogit_ship_shipment` returned `mutation_partial` with
  `classification: indeterminate` and `failed_step: shipped-event-append`,
  `git restore .backlogit/archive/` must NOT run at all and post-mode is invoked
  directly. See Post-Mode step 0.
...
* `HALT — shipped-event reconciliation required` — post-mode only (143-F, workspace-specific):
  ...
  and `failed_step: shipped-event-append`, so archival was deliberately halted before the
  archive collector ran. Missing archive files are the intended output, not lost files.
...
0. **Classify the ship result first (143-F, workspace-specific)**: Inspect the
   `backlogit_ship_shipment` result before scanning for archives. If it returned
   ...
   * This step applies only when the cascade op actually ran (the P-015 verified
...
* `src/autoharness/gates/shipment_closure.py` — the autoharness self-hosting repository's own `classify_shipment_close_path` implementation, reused by Step 0 of Safe-Close Mode **where installed**. This Go workspace has no such Python gate module; Step 0 implements the equivalent P-015 classification directly against `.backlogit/queue/` + `.backlogit/archive/`, as Safe-Close Mode Step 0(c) provides for.
