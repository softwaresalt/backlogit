...
* gate `decision` — must be `PASS` or `ADVISORY` (with explicit recorded authorization).
...
The gate is satisfied ONLY when `decision == PASS`, or `decision == ADVISORY` with an
explicit recorded authorization. A missing, unrecognized, or FAIL decision MUST halt.
...
* **ADVISORY**: Present findings to user; proceed if user confirms. Record the authorization explicitly in the plan's `## Plan Review` section.
...
4. Apply the contract:
   ...
   * `decision == ADVISORY` → gate satisfied ONLY when an explicit operator authorization
     is recorded in the `## Plan Review` section; otherwise **HALT** and re-run plan-review
     or require operator intervention.
