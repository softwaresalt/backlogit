...
* Check `src/` for undocumented modules or public APIs.
...
* Focus areas: Script parameter documentation in scripts/, file structure descriptions in docs/, example commands and their expected behavior, CLI usage documentation.
...
* Scan locations: scripts/ (scripts without README or usage docs), src/ (undocumented modules or public APIs), .github/skills/ (skills without adequate documentation).
...
* Execute available validation commands:
  * `python -m pytest tests/` for test suite validation.
  * `python -m mypy src/` for type checking (if configured).
  * Manual review against instructions files for markdown compliance.
...
| Command | Purpose |
| ------- | ------- |
| `python -m pytest tests/` | Test suite validation |
| `python -m mypy src/` | Type checking (if configured) |

If validation commands are unavailable, rely on manual review against instructions files.
