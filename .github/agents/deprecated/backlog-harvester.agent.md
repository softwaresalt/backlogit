...
* `${input:skip_plan:false}`: (Optional, defaults to `false`) When `true`, skip Phase 1 (impl-plan) and use the source document directly. Only valid when source is a `docs/exec-plans/` file that was already planned externally.
...
3. The impl-plan skill writes its output to `docs/exec-plans/{YYYY-MM-DD}-{slug}-plan.md`.
