...
Ensure every task in the target feature or chore is **harness-satisfied** before any implementation begins. This step runs once, up front — not in a loop.
...
2. Partition the task list into three sets:
   * **Already harnessed**: tasks carrying the `harness-ready` label — skip these.
   * **Exempt**: tasks carrying the `harness-exempt` label — evaluate each against P-002.1 **static intake** (see Step 2a) and skip scaffolding for every one that passes. A `covered-by` task's named owner is **not** exempt and belongs in the "needs harness" partition below.
   * **Needs harness**: tasks carrying neither label — scaffold these.
3. **Land declaration prerequisites before scaffolding their dependents.** A `declaration-only` exempt task contributes no harness, but the types and fields it declares are what a dependent task's harness compiles against. Scaffolding every dependent in one undifferentiated pass would deadlock a declaration-first DAG: the dependent's harness cannot compile until the declaration exists, and the declaration cannot land while this step forbids implementation. Break the deadlock by ordering, never by weakening the gate.
   * Identify every P-002.1-valid `declaration-only` task that a "needs harness" task depends on, using the dependency graph rather than prose.
   * **Run each identified task through the normal Step 4 task loop, in full and unchanged** — Step 4.1a claim-time gate (contract re-read, P-002.5 read-only screen, must-fail-before-deliverable probe, `EXEMPT_VERIFY_OK` marker handling), then Step 4.1b claim, Step 4.2 build-feature delegation with `harness_cmd` / `exempt_gate_cmd` / `exempt_class`, then Step 4.3 quality gates including the exempt completion gate, the P-002.4 class delta surface, and the status transition. This item selects *which* tasks execute early and in what order. It is **not** an alternative execution path, it defines no shortcut probe of its own, and it grants no exemption from any Step 4 gate. Checking only the final marker is insufficient and is not what this item authorizes.
   * Any Step 4 halt code raised inside this early pass halts here exactly as it would in the main loop — including `EXEMPT_CONTRACT_INCOMPLETE`, `EXEMPT_COMMAND_DESTRUCTIVE`, `EXEMPT_FALSE_GREEN`, `EXEMPT_MARKER_MISSING`, `EXEMPT_EVIDENCE_MISMATCH`, and `EXEMPT_DELTA_EXCEEDS_CLASS`. Do not resume harness generation with a halted prerequisite.
   * Then scaffold the dependents against the now-compilable declared shape, in dependency order.
   * This is an ordering rule only. No task with a harness obligation is implemented before its own harness is red, and no exempt task is ever scaffolded. If a task that is **not** `declaration-only` blocks a dependent's harness from compiling, halt and report rather than implementing ahead of the gate.
4. If any tasks need harnesses, invoke the **harness-architect** skill for that set only.
   ...
   * Do **not** pass P-002.1-valid exempt tasks to the skill. Scaffolding a red test for a valid exempt task is a P-002.1 producer-obligation violation.
5. After scaffolding completes, confirm every queued task is harness-satisfied — carrying `harness-ready`, or carrying `harness-exempt` and passing P-002.1 static intake. If any task is neither, halt and report the gap rather than proceeding with a partial set. Do **not** evaluate `covered-by` owner red evidence here; that is Step 4.1a's gate.
...
**This step evaluates static conditions only.** It runs *before* harness generation, so no
predecessor owner can have red evidence yet. Checking owner red evidence here would make
`covered-by` unsatisfiable by construction and deadlock the shipment at its own gate. Owner red
evidence is re-evaluated at claim time in Step 4.1a.

For each task carrying `harness-exempt`:
...
2. **Validate the class.** `harness_exemption_class` must be exactly one of `declaration-only`,
   `docs-only`, `verification-only`, `covered-by`. Anything else → halt with
   `EXEMPT_CLASS_UNRECOGNIZED`. `harness_exemption_reason` must be one non-empty line; missing →
   halt with `EXEMPT_CONTRACT_INCOMPLETE`.
...
5. **Classify the change objectively (P-002.4).** If the task's declared deliverable changes
   ...
   Declaration-only, docs-only, and verification-only tasks need no test and MUST NOT have one
   fabricated.
...
Step 2 scaffolding (unless it already carries `harness-ready`), so its red evidence exists by the
time Step 4.1a runs.

Behavior-changing tasks that are not covered by a valid, red-confirmed predecessor still require
`harness-ready` before implementation. Test-first semantics are unchanged for all non-exempt work.
...
Now that all tasks are harness-satisfied, construct the execution queue:

1. List all queued tasks for the target feature or chore that are harness-satisfied — carrying the `harness-ready` label, or carrying the `harness-exempt` label and passing the Step 2a static intake. With `backlogit`, the equivalent predicate is `status = 'queued' AND (labels LIKE '%harness-ready%' OR labels LIKE '%harness-exempt%')`, applied only after Step 2a has statically validated each exempt task. Queue membership is admission to the *queue*, not to the *build*: every exempt task still passes Step 4.1a before it is claimed.
...
* `harness_cmd`: The test command from the task's harness-ready metadata (e.g., `go test ./... --test {feature}_test`). For a `harness-exempt` task there is no red harness; pass the command the loop must drive from failing to passing:
  * `declaration-only`, `docs-only`, `verification-only` → the task's `exempt_verification_command`
...
4. **Harness-exempt completion gate (P-002.3 / P-002.4)** — for `harness-exempt` tasks only:
   ...
   * Run the P-002.4 **path pass** — `git diff --name-only {exempt_baseline_sha}..HEAD`, using the
     ...
     confirm the result is a subset of the delta surface for `exempt_class`. Any file outside it →
     halt with `EXEMPT_DELTA_EXCEEDS_CLASS`. A non-test `*.go` change under `docs-only` or
     `verification-only` → halt with `EXEMPT_BEHAVIOR_NO_OWNER`. Any `*_test.go` change under
     `covered-by` → halt with `EXEMPT_DELTA_EXCEEDS_CLASS`.
   * Run the P-002.4 **content pass** — `git diff {exempt_baseline_sha}..HEAD -- <each allowed
     ...
     `declaration-only`, a hunk that adds, removes, or edits a line inside a **pre-existing function
     body** → halt with `EXEMPT_BEHAVIOR_NO_OWNER` unless it is P-002.4's behavior-preserving
     re-expression **and** `exempt_gate_cmd` pins the pre-existing contract. Under
     `verification-only`, no hunk may weaken, delete, rename, or narrow the selector of a
     pre-existing assertion. A path-only check is **not** sufficient: without the content pass a
     `declaration-only` task can change behavior inside a file its own contract names.
