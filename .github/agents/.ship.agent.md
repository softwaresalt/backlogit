...
* invoke the modular `harness-architect` skill for harness generation (P-002/P-004), passing only tasks that are not already harness-satisfied
...
2. Partition the task list into three sets:
   ...
   * **Exempt**: tasks carrying the `harness-exempt` label — evaluate each against P-002.1 (see Step 2a) and skip scaffolding for every one that passes.
...
4. After scaffolding completes, confirm every queued task is harness-satisfied — carrying `harness-ready`, or carrying `harness-exempt` and passing P-002.1. If any task is neither, halt and report the gap rather than proceeding with a partial set.

#### Step 2a: Harness-Exempt Evaluation (P-002.1, fail-closed)
...
1. **Read the declared exemption class.** It must be one of `declaration-only`, `docs-only`, `verification-only`, or `covered-by <owner-id>`. Anything missing, unreadable, or outside that closed vocabulary → halt with `EXEMPT_CLASS_UNRECOGNIZED`.
2. **Read the declared reason.** One line stating why no red harness is scaffolded. Missing → halt with `EXEMPT_CLASS_UNRECOGNIZED`.
3. **Confirm contract membership.** The governing plan, feature, or shipment contract must enumerate a closed exempt set containing this task. Not represented → halt with `EXEMPT_NOT_IN_CONTRACT`.
4. **Classify the change.** If the task changes behavior and its class is not `covered-by` → halt with `EXEMPT_BEHAVIOR_NO_OWNER`. Declaration-only, docs-only, and verification-only tasks need no test and MUST NOT have one fabricated.
5. **Validate the predecessor owner** when the class is `covered-by <owner-id>`: the owner exists in the same release unit, is a declared dependency (direct or transitive) of this task, is itself `harness-ready` rather than exempt, and its red evidence is confirmed (`Compilation: PASS`, `Red Phase: CONFIRMED`) with its harness commit landed. Any failure → halt with `EXEMPT_OWNER_INVALID` or `EXEMPT_OWNER_NOT_RED`.
6. **Admit or halt.** A task passing every check is harness-satisfied — schedule it in dependency order like any other queued task, without scaffolding. Otherwise halt, record the P-002.2 code through P-005 telemetry, and return the task to the operator or to Stage for a contract amendment. Do not relabel it `harness-ready` and do not scaffold a substitute harness.
...
1. List all queued tasks for the target feature or chore that are harness-satisfied — carrying the `harness-ready` label, or carrying the `harness-exempt` label and passing the Step 2a evaluation. With `backlogit`, the equivalent predicate is `status = 'queued' AND (labels LIKE '%harness-ready%' OR labels LIKE '%harness-exempt%')`, applied only after Step 2a has validated each exempt task.
...
* `harness_cmd`: The test command from the task's harness-ready metadata (e.g., `go test ./... --test {feature}_test`). For a P-002.1-valid `harness-exempt` task there is no red harness; pass the task's declared verification command instead (`docs lint` for `docs-only`, the compile check for `declaration-only`, the named guard selector for `verification-only`, or the predecessor owner's harness command for `covered-by`).
