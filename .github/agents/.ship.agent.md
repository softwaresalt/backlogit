...
3. **Run the must-fail-before-deliverable probe (P-002.3).** Execute
   ...
   * It MUST fail. Non-zero exit is failure; so is any false-green signal even at exit 0 —
     `[no tests to run]`, `testing: warning: no tests to run`, `no test files`, or zero matching
     `--- PASS: <name>` lines for a named-selector command.
   * If it **succeeds** before any work is done → halt with `EXEMPT_FALSE_GREEN`. Do not implement,
     do not weaken the command, and do not assume the deliverable already landed.
   * Record the observed pre-work output as the task's red-equivalent evidence.
4. **Proceed.** Only after 1–3 pass may the task be claimed and dispatched to build-feature.
...
4. **Harness-exempt completion gate (P-002.3 / P-002.4)** — for `harness-exempt` tasks only:
   * Run `exempt_gate_cmd` once. It MUST exit 0 **and** carry no false-green signal **and** match
     ...
     empty selector, skipped package, no-op probe) → halt with `EXEMPT_EVIDENCE_MISMATCH`.
