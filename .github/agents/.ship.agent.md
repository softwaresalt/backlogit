...
`build-feature` is not driving a red command to green. Dispatch it with `harness_cmd` set to the
declared `red_selector_command` and the explicit instruction that the deliverable is the **landed,
compiling, failing** harness: the loop lands the harness, confirms it compiles
(`go test -run=^$ -count=1 ./...`) and fails on assertions rather than on a build error, and stops
...
`red_selector_command` observed RED, red evidence recorded* — never "scoped command green". Record
the observed failure output as its evidence, add its selector to `open_red_deliverables`, and note
the declared green-makers and `green_maker_closes_wave` in the completion comment so the deferral
at Step 4.6 is traceable to a stated closing wave.
