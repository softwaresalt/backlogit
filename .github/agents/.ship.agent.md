...
inside Step 4. This step establishes the manifest member set and the wave partition that Step 4
iterates.

1. List **every** member of the shipment manifest for the target feature or chore, at every status.
   This is the frozen member set `M`. Record `count(M)`; it is the wave budget. `M` is **immutable
   for the life of the release unit** — Step 4.0 never re-derives it, because `return_blocked`
   removes a returned member from the shipment's live `items` list and the member must stay visible
   to the scheduler. Only a Stage manifest amendment may change `M`.
2. Read the dependency graph. With `backlogit`, prefer the dependency operation over inferring
   order from prose or from backlog sort order.
3. Verify the graph is **acyclic** (Kahn topological sort over all of `M`). A cycle → halt with
...
4. Compute the wave partition: wave 1 is every member with no unfinished dependencies; wave *k+1*
...
5. If `M` is empty, halt and report — there is nothing to build.
...
frozen manifest set `M` is entirely terminal (`done` or `archived`). An empty ready frontier is
never a completion condition — see P-002.6.
...
1. **Snapshot the whole manifest from live state.** Take **one status+dependency snapshot call**
   ...
   CLI/MCP-list forms and the `WAVE_SNAPSHOT_UNRELIABLE` fail-closed halt. `M` was frozen at Step
   0.5 and is **immutable**: do not re-derive it from the shipment's live `items` list, because
   `return_blocked` removes a returned member from that list and the member must stay visible to
   this loop. Partition `M` into `queued`, `active`, `blocked`, and `terminal` (`done` or
   `archived`).
2. **Halt on any blocked member.** If `blocked` is non-empty → halt with `WAVE_MEMBER_BLOCKED`.
   Report, in stable task-ID sort order: the wave index; `count(M)`, `count(terminal)`,
   `count(queued)`, `count(active)`, `count(blocked)`; each blocked member ID with its recorded
   `blocked_reason`; the **dependency impact** of each — the transitive members that can never
   reach the frontier while it is blocked; and whether the `return_blocked` operation has already
   been invoked for it. If the member is blocked, has not been returned, and shipment semantics
   call for it, invoke `backlogit_return_blocked` and **record** that invocation in the report. A
   blocked member never leaves `M`, never re-enters `ready_k`, and never permits a completion
   claim.
3. **Halt on an active residual.** If any member is still `active` at wave admission, it is an
...
4. **Check for completion.** If `terminal = M` — every member of the frozen manifest is `done` or
   `archived` — the release unit is complete; exit the wave loop. This is the **only** completion
   condition. An empty `ready_k` is never one.
5. **Compute the frontier.** `ready_k = { t in queued : every dependency of t is terminal }`.
...
6. **Report deterministically on halt.** A `WAVE_NO_PROGRESS`, `WAVE_CYCLE_DETECTED`, or
   `WAVE_MEMBER_BLOCKED` report MUST carry the full census in item 2's form (plus the cycle path
   for `WAVE_CYCLE_DETECTED`). "No ready tasks" alone is not a report. Record it through P-005
   telemetry and return the release unit to the operator or to Stage for a graph or manifest
   amendment.
7. **Enforce the wave budget.** If the wave index exceeds `count(M)` from Step 3 → halt with
   `WAVE_BUDGET_EXCEEDED`. Exceeding it means a scheduler defect, not a graph property.
8. **Classify and scaffold the wave.** Run **Step 2a** (static intake) and then **Step 2**
   ...
   a partial wave.
9. **Execute the wave, task-scoped.** Run Steps 4.1a → 4.5 for each member of `ready_k`. Only
   members of the current wave may be claimed. Each task is driven green against **its own scoped
   verification command** — a sibling's still-red harness is expected until that sibling is built
   and MUST NOT fail the task under build (see Step 4.2 and Step 4.3).
10. **Converge the wave.** When every member of `ready_k` is individually green and completed, run
    **Step 4.6** (wave convergence gate). Only after it passes may the wave index be incremented
    and control return to item 1.

**Blocked-injection validation.** Before this scheduler is relied on for a release unit, replay its
wave loop once with one member's status forced to `blocked`. The replay MUST halt with
`WAVE_MEMBER_BLOCKED`, report the dependency impact, keep the member in `M`, and never report
completion. A replay that completes or that silently drops the member is a scheduler defect.

**A task whose declared deliverable *is* a red harness** — a `covered-by` owner such as a
harness-only contract unit — completes when its harness has landed and been confirmed red. Do
**not** drive it green in its own wave; its dependent does that in a later wave. Driving it green
here would erase the very red the dependent's `covered-by` claim needs at claim time.
...
When the `agent-intercom` capability pack is installed, broadcast
`[SHIP] Wave {k}: {count} task(s) admitted ({scaffold} to scaffold, {exempt} exempt)` at item 8 and
`[SHIP] Wave {k} converged, {remaining} member(s) non-terminal` at item 10.
...
* `wave_scoped`: `true` whenever this dispatch comes from a wave (Step 4.0 item 9) — which is
...
* `sibling_red_selectors`: the closed, explicit list of `-run` selectors belonging to the **other**
  non-exempt members of the current wave that have not yet been built. This is the **entire**
  tolerated-red set. The skill may not widen it, and any failure outside it is a real failure.
...
**Sibling red is expected inside a wave.** Step 4.0 item 8 leaves every non-exempt member of the
wave red at once, and the members are then built one at a time. The first task built would fail any
repo-wide suite on its siblings' harnesses no matter how correct it is. That is why the per-task
loop is scoped and the full suite is relocated to Step 4.6 — not skipped, and not weakened.
...
2. **Format**: `gofmt -l .`
3. **Task-scoped test suite**: run the task's own scoped `harness_cmd`, plus any package whose
   tests were already green before this wave began. **Do not run a repo-wide or full-package suite
   here when it would include a current-wave sibling's still-red harness** — that gate is not
   stricter, it is unsatisfiable, and the full suite runs at Step 4.6 instead. The tolerated-red set
   is exactly `sibling_red_selectors`; a failure outside it is a real failure and blocks the task.
...
Steps 4.1a–4.5 (or carries an appropriate terminal status), and **before** the wave index advances.
This is where the full suite that Steps 4.2/4.3 deliberately scoped away is executed.

1. **Confirm the wave is individually converged.** Every member of `ready_k` is `done` (or
   `archived`), and no member is `active` or `blocked`. A member still `active` → halt with
   `WAVE_NO_PROGRESS` (detail: `active residual`). A member `blocked` → halt with
   `WAVE_MEMBER_BLOCKED` and run the Step 4.0 item 2 report. Do **not** converge around an
   unfinished member.
2. **Run the full repository suite**: `go test ./...`. Every current-wave harness is now green
   because every member has been built, so the tolerated-red set is **empty** at this gate. No
   selector, no `-short`, no skip.
3. **Run the repo-wide static gates**: `go vet ./...`, `golangci-lint run`, `gofmt -l .`.
4. **On failure, remediate inside the current wave.** Halt, diagnose, and fix within this wave's
   members. Do **not** advance the wave index, do **not** admit the next wave, and do **not** carry
   a failing suite forward as "expected red" — every sibling has been built, so nothing in this
   wave is legitimately red any more.
5. **On success, advance.** Increment the wave index and return to Step 4.0 item 1 for a fresh
   snapshot.
...
When the `agent-intercom` capability pack is installed, broadcast
`[SHIP] Wave {k} convergence gate: full suite {PASS|FAIL}` on completion of item 2.
...
| Wave convergence gate failures (P-002.6, Step 4.6) | 3 | Halt, remediate inside the current wave, write checkpoint; never advance the wave index on a failing full suite |
