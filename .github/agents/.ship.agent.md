...
`build-feature` is not driving a red command to green. Dispatch it with `red_deliverable: true` and
`harness_cmd` set to the declared `red_selector_command` verbatim. That input routes the skill to
its **Step 0.5** branch, which replaces the generic loop rather than modifying it: the branch lands
the declared harness, confirms it compiles (`go test -run=^$ -count=1 ./...`), requires the selector
to fail on **named assertions** rather than on a build error or a no-tests-to-run signal, and stops
there. Do not iterate toward green, and do not accept a green result — a red deliverable that
passes on landing was never red.
...
closed at its own boundaries and returns, rather than repairs, four conditions:
`WAVE_RED_MAPPING_UNRESOLVED` for a dispatch defect, `WAVE_RED_DELIVERABLE_EARLY_GREEN` for a
selector that passes before or after landing, `WAVE_RED_DELIVERABLE_PRELANDED` for a harness already
landed and already failing at dispatch, and `WAVE_RED_DELIVERABLE_VACUOUS` for a selector that
matches no test after landing. Each is a P-002.2 halt, not a fix iteration: record it through P-005
telemetry and return the task for a contract amendment.

Step 0.5e returns the red-evidence report — the executed selector, the observed failing function
names and exit code, the passing repo-wide compile check, and the declared `green_maker_tasks` and
`green_maker_closes_wave`. Step 4.5 builds the `open_red_deliverables` entry from that report, and
Step 4.6 items 4 and 5 re-confirm the entry at every later gate, so an absent or partial report is a
halt rather than a completion.
...
**A red-deliverable task completes red.** Its criterion at this step is *harness landed,
`red_selector_command` observed RED, red evidence recorded* — never "scoped command green". Build
the record from `build-feature`'s Step 0.5e report rather than re-deriving it: record the observed
failure output as its evidence, add its selector to `open_red_deliverables`, and note the declared
green-makers and `green_maker_closes_wave` in the completion comment so the deferral at Step 4.6 is
traceable to a stated closing wave. An absent or partial Step 0.5e report leaves the entry
unaccounted for and is a halt, not a completion.
