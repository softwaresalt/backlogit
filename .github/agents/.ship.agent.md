...
4. Invoke the **harness-architect** skill for the "needs harness" partition of **this wave only**.
   * Require compilable but failing harnesses, structural stubs, and successful
     `go test -run=^$ -count=1 ./...` verification after scaffolding.
...
inside Step 4. This step establishes the task set and the wave partition that Step 4 iterates.

1. List all `queued` tasks for the target feature or chore. This is the task set `T`. Record
   `count(T)`; it is the wave budget.
...
3. Verify the graph is **acyclic** (Kahn topological sort over all of `T`). A cycle → halt with
...
4. Compute the wave partition: wave 1 is every task with no unfinished dependencies; wave *k+1* is
   every remaining task whose dependencies are all in waves 1..*k*. Record the partition as the
   expected schedule so a deviation at Step 4.0 is visible rather than silent.
5. If `T` is empty, halt and report — there is nothing to build.
...
Step 4 is an **outer loop over waves** wrapping the per-task loop. Repeat Steps 4.0 → 4.5 until no
queued tasks remain.
...
1. **Recompute the frontier from live state.** `ready_k = { t in queued : every dependency of t is
   done }`. Requery the backlog rather than trusting the Step 3 partition — a task returned
   blocked, archived, or added since Step 3 changes the frontier, and a stale partition would
   claim a task whose dependencies are unfinished. Requery with **one status+dependency snapshot
   call**, never a per-task lookup repeated over the frontier — see P-002.6's snapshot recipe for
   the SQL-join and CLI/MCP-list forms and the `WAVE_SNAPSHOT_UNRELIABLE` fail-closed halt.
2. **Halt on no progress.** If `ready_k` is empty while queued tasks remain → halt with
   `WAVE_NO_PROGRESS`. If the residual set contains a cycle → halt with `WAVE_CYCLE_DETECTED`.
   Never retry, never widen the frontier, and never claim a task with an unfinished dependency.
3. **Report deterministically on halt.** A `WAVE_NO_PROGRESS` or `WAVE_CYCLE_DETECTED` report MUST
   list, in stable task-ID sort order: the wave index, `count(done)`, `count(queued)`, every
   blocked task ID, and for each the exact unfinished dependency IDs (plus the cycle path for
   `WAVE_CYCLE_DETECTED`). "No ready tasks" alone is not a report. Record it through P-005
   telemetry and return the release unit to the operator or to Stage for a graph amendment.
4. **Enforce the wave budget.** If the wave index exceeds `count(T)` from Step 3 → halt with
   `WAVE_BUDGET_EXCEEDED`. Exceeding it means a scheduler defect, not a graph property.
5. **Classify and scaffold the wave.** Run **Step 2a** (static intake) and then **Step 2**
   (harness generation) over `ready_k` only. On return, every member of `ready_k` is
   harness-satisfied; if any is not, halt and report rather than building a partial wave.
6. **Execute the wave.** Run Steps 4.1a → 4.5 for each member of `ready_k`, unchanged. Only
   members of the current wave may be claimed.
7. **Advance.** When every member of `ready_k` is `done`, increment the wave index and return to
   item 1. Stop when no queued tasks remain.
...
When the `agent-intercom` capability pack is installed, broadcast
`[SHIP] Wave {k}: {count} task(s) admitted ({scaffold} to scaffold, {exempt} exempt)` at item 5 and
`[SHIP] Wave {k} complete, {remaining} task(s) queued` at item 7.
...
* `harness_cmd`: The test command from the task's harness-ready metadata (e.g., `go test ./... --test {feature}_test`). For a `harness-exempt` task there is no red harness; pass the command the loop must drive from failing to passing:
...
| Waves per release unit (P-002.6) | task count of the release unit | Halt with `WAVE_BUDGET_EXCEEDED`, write checkpoint, report the wave index and residual queue |
