...
3. If any tasks need harnesses, invoke the **harness-architect** skill for that set only.
...
4. After scaffolding completes, confirm every queued task is harness-satisfied — carrying `harness-ready`, or carrying `harness-exempt` and passing P-002.1 static intake. If any task is neither, halt and report the gap rather than proceeding with a partial set. Do **not** evaluate `covered-by` owner red evidence here; that is Step 4.1a's gate.
...
Run this step for every task carrying `harness-exempt`, immediately before it is claimed. It is
mandatory and it is the gate that owns the conditions Step 2a could not evaluate. Skip it for
`harness-ready` tasks.
...
5. **Proceed.** Only after 1–4 pass may the task be claimed and dispatched to build-feature.

Record any halt through P-005 telemetry with the P-002.2 code and return the task to the operator
or to Stage for a contract amendment.
