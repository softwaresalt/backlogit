...
5. **Proceed.** Only after 1–4 pass does Step 4.1b claim the task and dispatch it to build-feature.
...
4. **Harness-exempt completion gate (P-002.3 / P-002.4)** — for `harness-exempt` tasks only:
   ...
   * Compute the task's changed-file set (`git diff --name-only <pre-task-commit>..HEAD`) and
     confirm it is a subset of the P-002.4 delta surface for `exempt_class`. Any file outside it →
