...
2. **Run the always-on part of the gate.** First recompute `open_red_deliverables_k` after this
   ...
   * The `red_selector_command` of **every entry still open in the recomputed
     ...
     whose selector passes here is the same contract violation — report it rather than advancing;
     it cannot be remediated inside this wave's members, because the entry and its green-makers
     belong to other waves. An entry this wave *closed* is not run here: its selector is expected
     green and is covered by the unfiltered suite the moment the set empties.
