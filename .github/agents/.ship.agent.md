...
2. **Freeze the task-type wave set `M`.** Read the shipment artifact's complete
   ...
   * **Fail closed.** If the shipment list cannot be read, any listed ID or type cannot be
     resolved, neither source can produce a task list, or the **filtered shipment task IDs** and a
     covering-unit task enumeration are both available and disagree, halt with
     ...
     never infer `M` from prose, a plan table, backlog sort order, or the current ready frontier.
...
9. **Replay the scheduler before relying on it.** Run the tracked, read-only contract simulation
   ...
   `WAVE_SIM_OK`. It reads the real shipment manifest, filters task IDs into `M`, reports excluded
   non-task IDs, verifies every red-deliverable key, and runs in-memory mutation checks, so a
   drifted fixture or regressed halt path is caught here rather than mid-release. A failing or
   drifted simulation blocks the schedule from being used (P-002.6).
...
1. **Snapshot the whole task wave set from live state.** Take **one status+dependency snapshot
   call** over every task in frozen `M` — exactly `count(M)` rows, not `count(S)` shipment members;
   never a per-task lookup repeated over the frontier, and never filtered by status. See P-002.6's
   snapshot recipe for the SQL-join and CLI/MCP-list forms and the
