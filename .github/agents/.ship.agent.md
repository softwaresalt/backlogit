...
**This step does not freeze the manifest member set `M`.** Intake validates and claims the
...
inside Step 4. This step establishes the status model, the frozen manifest member set, the
...
1. **Read the configured status model.** Load the workspace status catalog
   ...
   (`.autoharness/backlog-registry.yaml` → `status_values`), and confirm the archive lifecycle
   terminal (`archived`). Record three sets: **executable** (`queued`, `active`, `blocked`),
...
2. **Freeze the manifest member set `M`.** List **every** member of the shipment manifest for the
   target feature or chore, at every status. That list is `M`. Record `count(M)`; it is the wave
   budget. `M` is **immutable for the life of the release unit** — Step 4.0 never re-derives it,
   because `return_blocked` removes a returned member from the shipment's live `items` list and the
   member must stay visible to the scheduler. Only a Stage manifest amendment may change `M`.
   * **Non-shipment fallback.** When the registry does not advertise `features.shipments: true`, or
     Ship was invoked without a shipment, freeze `M` from the covering release unit's declared
     child enumeration instead — every task whose `parent_id` is the release unit, at every status
     — and write that enumeration verbatim into the session record and the next checkpoint. It is
     the same object: one explicit, closed, recorded member list.
   * **Fail closed.** If neither source can produce a member list, or a shipment manifest and a
     covering-unit enumeration are both available and **disagree**, halt with
     `WAVE_MANIFEST_UNAVAILABLE` (P-002.2). Never infer `M` from prose, a plan table, backlog sort
     order, or the current ready frontier.
3. **Build the red-deliverable mapping.** For every member, read the
   ...
   contributes an entry `(task, red_selector_command, green_maker_tasks, green_maker_closes_wave)`
   to the mapping Step 4.6 consumes. Validate each entry mechanically: the green-maker list is
   non-empty, every ID is in `M`, none is the task itself, and every one lands in a strictly later
   wave than the task. Any failure — absent block on a red-deliverable member, empty list, unknown
   ID, self-reference, not-strictly-later, or two blocks disagreeing about one task — halts with
   `WAVE_RED_MAPPING_UNRESOLVED` (P-002.2). Never infer the mapping from prose, labels, or
   dependency direction.
4. Read the dependency graph. With `backlogit`, prefer the dependency operation over inferring
   order from prose or from backlog sort order.
5. Verify the graph is **acyclic** (Kahn topological sort over all of `M`). A cycle → halt with
...
6. Compute the wave partition: wave 1 is every member with no unfinished dependencies; wave *k+1*
...
7. If `M` is empty, halt and report — there is nothing to build.
8. **Replay the scheduler before relying on it.** Run the tracked, read-only contract simulation
   (`pwsh -NoProfile -File scripts/wave-scheduler-sim.ps1 -VerifyAgainstQueue`) and require
   `WAVE_SIM_OK`. It re-derives this manifest from the live queue, so a drifted fixture, a missing
   red-deliverable contract, or a regressed halt path is caught here rather than mid-release. A
   failing or drifted simulation blocks the schedule from being used (P-002.6).
...
frozen manifest set `M` is entirely terminal-success (`done` or `archived`). An empty ready frontier
...
1. **Snapshot the whole manifest from live state.** Take **one status+dependency snapshot call**
   over every member of the frozen manifest set `M` — never a per-task lookup repeated over the
   frontier, and never filtered by status. See P-002.6's snapshot recipe for the SQL-join and
   CLI/MCP-list forms and the `WAVE_SNAPSHOT_UNRELIABLE` fail-closed halt. `M` was frozen at Step 3
   and is **immutable**: do not re-derive it from the shipment's live `items` list, because
   `return_blocked` removes a returned member from that list and the member must stay visible to
   this loop. Partition `M` against the Step 3 status model into `queued`, `active`, `blocked`,
   `terminal_success` (`done` or `archived`), and `unsupported` (everything else).
...
(`pwsh -NoProfile -File scripts/wave-scheduler-sim.ps1 -VerifyAgainstQueue`, Step 3 item 8). The
...
* `green_regression_cmds`: the task's declared, closed list of additional scoped commands that must
  pass alongside `harness_cmd` — empty unless the task declares otherwise. This list replaces the
  withdrawn "any package that was already green before this wave began" allowance: the loop runs
  exactly the declared commands and never a package of the implementer's choosing.
...
2. **Format**: `gofmt -l .`
3. **Task-scoped test suite**: run the task's own scoped `harness_cmd` plus its declared
   `green_regression_cmds` — the closed, task-declared list, empty unless the task declares
   otherwise — and nothing else. **Do not run a repo-wide or full-package suite here**, and do not
   add packages by judgement: the withdrawn "any package that was already green before this wave
   began" allowance was implementer discretion wearing a gate's clothing, unenumerated and
   unreviewable. The full suite runs at Step 4.6 instead. The tolerated-red set is exactly
...
2. **Run the always-on part of the gate.** These run at every wave, unconditionally:
   ...
   * The wave's **closed list of declared scoped commands**: every member's `harness_cmd` and its
     ...
     that has gone green early is a contract violation; report it rather than advancing.
3. **Decide the full suite from the open-red set.** Recompute `open_red_deliverables_k` after this
   wave's completions (Step 4.0 item 9 rules; `archived` does not close a green-maker).
   ...
   * **Non-empty** → **defer** the full suite to the first convergence gate at which the set is
     ...
     scoped command have run repo-wide or task-scoped and passed; the only failing selectors in the
...
| Waves per release unit (P-002.6) | `count(M)` — the frozen manifest member count | Halt with `WAVE_BUDGET_EXCEEDED`, write checkpoint, report the wave index and the residual census |
