...
3. **Land declaration prerequisites before scaffolding their dependents.** A `declaration-only` exempt task contributes no harness, but the types and fields it declares are what a dependent task's harness compiles against. Scaffolding every dependent in one undifferentiated pass would deadlock a declaration-first DAG: the dependent's harness cannot compile until the declaration exists, and the declaration cannot land while this step forbids implementation. Break the deadlock by ordering, never by weakening the gate.
   ...
   * Execute those declaration tasks first — declaration commit plus their `TestU<unit>Guard_*` green-step guards — and verify each with its `exempt_verification_command`, requiring the `EXEMPT_VERIFY_OK` marker exactly as Step 4.1a does.
...
4. **Run the must-fail-before-deliverable probe (P-002.3).** Execute
   ...
   * It MUST fail. Non-zero exit is failure; so is any false-green signal even at exit 0 —
     `[no tests to run]`, `testing: warning: no tests to run`, `no test files`, zero matching
     `--- PASS: <name>` lines for a named-selector command, or exit 0 without the declared
     `EXEMPT_VERIFY_OK:{task_id}` marker.
   * If it **succeeds** before any work is done — exit 0 with the marker present — → halt with
     `EXEMPT_FALSE_GREEN`. Do not implement, do not weaken the command, and do not assume the
     deliverable already landed. An exit 0 that lacks the marker is `EXEMPT_MARKER_MISSING`, not a
     pass and not a reason to proceed either.
