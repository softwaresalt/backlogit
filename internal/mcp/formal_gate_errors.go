...
func formalGateErrorResult(err error) (*mcplib.CallToolResult, bool) {
	...
	case errors.Is(err, corerrors.ErrGateKeyAbsent):
		return marshalGateError(map[string]any{
			...
			"retryable":       false,
			"remediation":     "Set BACKLOGIT_GATE_EVIDENCE_KEY to a base64- or hex-encoded key of at least 32 decoded bytes, then retry.",
		}), true
	case errors.Is(err, corerrors.ErrGateKeyInvalid):
		return marshalGateError(map[string]any{
			...
			"min_key_bytes":   minFormalGateKeyBytes,
			"retryable":       false,
			"remediation":     "BACKLOGIT_GATE_EVIDENCE_KEY must decode (base64 or hex) to at least 32 bytes. Fix the value, then retry.",
		}), true
	case errors.Is(err, corerrors.ErrFormalReportInvalid):
		return marshalGateError(map[string]any{
			...
			"message":     err.Error(),
			"retryable":   false,
			"remediation": "The gate report must be valid JSON with at least one reviewers[] entry carrying a non-blank persona and decision. Re-run the gate with a conforming report, then retry.",
		}), true
	case errors.Is(err, corerrors.ErrProofInvalid):
		return marshalGateError(map[string]any{
			...
			"message":     err.Error(),
			"retryable":   false,
			"remediation": "The recorded formal-gate-evidence proof did not verify (tampered field, wrong key, replayed counter, or a superseding later event). Investigate before retrying; a bare retry will not resolve a tampered or replayed proof.",
		}), true
	case errors.Is(err, corerrors.ErrProofUnverifiable):
		return marshalGateError(map[string]any{
			...
			"message":     err.Error(),
			"retryable":   false,
			"remediation": "The formal-gate-evidence proof could not be evaluated (missing proof fields or a canonicalization failure). Verify BACKLOGIT_GATE_EVIDENCE_KEY is set correctly and the evidence was recorded under formal enforcement, then retry.",
		}), true
	case errors.Is(err, corerrors.ErrFormalGateRequired):
		...
		return marshalGateError(map[string]any{
			...
			"message":     err.Error(),
			"retryable":   false,
			"remediation": "Formal gate evidence is required but could not be produced or verified. See the message for the specific cause before retrying.",
		}), true
	default:
		return nil, false
	}
