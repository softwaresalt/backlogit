	...
	s.addTool(
		mcplib.NewTool("backlogit_create_checkpoint",
			mcplib.WithDescription("Save a session state checkpoint from a JSON state_dump. The top level "+
				"and the nested progress object are a CLOSED schema namespace: the only legal top-level keys "+
				"you may supply at create are schema_version, agent, session_id, phase, status, created_at, "+
				"updated_at, context, progress, and resume_hint, and the only legal keys inside progress are "+
				"tasks_completed, tasks_remaining, files_modified, and decisions; any other key at either level "+
				"fails the call as validation_failed with an unknown_fields list naming the offending key "+
				"path(s). The four disposition fields (disposition, disposition_reason, disposition_operator, "+
				"disposition_at) are part of the schema but are RESERVED and administrative: they are set only "+
				"by backlogit_abandon_checkpoint, never at create, and supplying one here is rejected as an "+
				"unknown field. The context object is the OPEN counterpart: shipment_id, feature_id, task_ids, "+
				"and branch are modeled, but any other key you supply there survives the create round-trip "+
				"unchanged. The successful result reports context_keys: the exact list of context key names "+
				"persisted to disk."),
