...
**Decision:** a TWO-PHASE lock protocol (final canonical form — supersedes any earlier sequential ordering): **Phase A** acquire `lockShipmentMembership(shipment)` (HELD across ALL phases — blocks `AddItemToShipment`-path manifest changes); compute the member set + the scalar request-identity digest; **Phase B** acquire ONE globally-sorted `lockArtifactMutations({shipment}∪members)` batch (identical order to every caller — `shipment.go:447-471`; NEVER shipment-then-members sequentially, which inverts against a concurrent sorted batch → deadlock), run the identity/location gate + classify on request-identity ONLY (non-2d branches finalize here with NO evidence I/O — replay reads no closure); **Phase C** (branch 2d only) run the SLOW delivery-evidence I/O with NO artifact lock and NO item-log lock held (only membership lock A); **Phase D** (append branches, GLOBAL ORDER A→C→B) acquire the item-log lock C FIRST, THEN re-acquire the sorted artifact batch B (this C-before-B order matches `AssociateCommit`, which takes the item-log lock at `commits.go:102` then the artifact-mutation lock via `persistArtifact` at `shipment.go:839-844` — taking B-before-C would DEADLOCK; a concurrency test MUST prove no deadlock), RE-READ the manifest under B and assert the member set EQUALS the Phase-A set — ANY change → FAIL CLOSED to `indeterminate` (because `UpdateArtifact` can replace `custom_fields.items` under the ARTIFACT lock without the membership lock, `artifacts.go:514-518,604-606`, the membership lock alone is insufficient and this equality check is required) — RELOAD frontmatter + take the rollback snapshot (after C and B are held), AUTHORITATIVELY re-classify (TOCTOU), re-check member terminality, and hold the item-log lock C across the final reclassify + IN-PLACE archive-file rewrite (via a dedicated HANDLE-RELATIVE atomic writer bound to the validated archive-directory handle, NOT path-based `replaceFileWithOptions` — see plan U1) + the durable event append (the 167.007 append primitive runs UNDER this held lock and does NOT re-acquire it). Snapshot file BYTES + the SQLite index row (via the 167.016 primitive; absent-row restore is a NON-CASCADING single-row delete, not DeleteItemCascade) before mutation. Release the artifact batch B, then the item-log lock C, then the membership lock A (reverse of the A->C->B acquisition order). Slow bounded I/O is NEVER held under an artifact batch. The event append is the LAST step via a DURABLE fsync-backed appender whose HANDLE-RELATIVE item-log lock is a DISTINCT sidecar namespace (not an artifact-mutation lock): on `ErrWriteIndeterminate` (`stream.go:207-210`) report indeterminate (NO second append); on `ErrWriteNotApplied` roll back file+index; doctor `detectMissingShippedEvents` is the durable detector (no synthetic forward-recovery event to the same log). **Rollback triggers** (documented, distinguishing write-outcome class): a FRONTMATTER-write `ErrWriteNotApplied` (proven not written) → restore file+index snapshot (upsert-or-delete by original row-presence); a FRONTMATTER-write `ErrWriteIndeterminate` (rename may already have committed `archived_status: shipped`) → report `indeterminate` and DO NOT restore (restoring the snapshot could OVERWRITE an already-committed repair — explicitly forbidden), leaving a later replay to re-read the durable state; conflicting-history discovered under lock → abort before write; indeterminate event write → indeterminate result + operator remediation guidance. (Full mechanics + the total state classifier are in the plan.)
