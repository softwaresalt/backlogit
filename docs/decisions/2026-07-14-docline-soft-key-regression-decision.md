...
- **gofmt remediation scope boundary (gate runs; no-new-violations criterion).** *Principle/heuristic:* quality-gate honesty + width isolation — the authoritative `gofmt -l .` gate is run (never skipped), and this shipment is gated on introducing no new formatting violations (its changed Go files must be absent from `gofmt -l .` output) while unrelated pre-existing debt is tracked separately. *Justification:* the repository carries pre-existing `gofmt` debt (26 unrelated files at authoring time); requiring this width-isolated shipment to also remediate them would force unrelated cleanup and silently expand blast radius. *Rejected simpler alternative:* blocking this shipment until repo-wide `gofmt -l .` emits nothing (rejected because it couples this bounded guard to unrelated cleanup and violates width isolation; the pre-existing debt is instead captured as a separate tracked remediation backlog item, not skipped or accepted).
