...
doc_type: decision
description: 'Decision to guard live tracked corpus and hermetic value semantics, and to correct the docline base/extension seam so unknown top-level extension keys (backlogit-owned size metadata) are preserved in place rather than folded (relocated) under the docline namespace — changing both internal/docline/normalize.go (stop folding) and internal/docline/frontmatter.go (BaseFrontmatter carries and re-emits unknown top-level keys, closing the latent top-level drop) — opening the base JSON schema (schemas/docline/base-frontmatter-v1.schema.json) to accept producer-owned top-level extension properties without validating their semantics (backlogit derived contract owns size validation), and rewriting the authoring guide to the inheritance model, gated behind a RED-first integration regression, while preserving production Scope and protected scratch state; no realpath/symlink filesystem containment is implemented.'
docline:
...
- Production docline containment is **lexical-only** (`core.SafeResolve`) and `ApplyMigration` explicitly adds **no** symlink-based realpath containment, so a test-only task cannot deliver external-target rejection without unrelated new production behavior. The base docline contract is open/extensible, and `size`/`size_source`/`size_ruleset_version` are optional backlogit-owned extension keys docline never owns. **However, the current production seam contradicts that contract, and the current-HEAD behavior must be stated honestly**: `internal/docline/normalize.go` folds every non-contract top-level key under the `docline` namespace (`foldUnderDocline`), driven by the closed `contractFields` set in `internal/docline/policy.go`; and `internal/docline/frontmatter.go` serializes through `BaseFrontmatter.ToMap()`, which emits only base-contract fields plus `docline`. At current HEAD `FromMap`/`ToMap` **preserve** that nested `docline` map, so the folded keys are **relocated** under `docline` (moved off the top level) and **not dropped today**; the missing top-level carrier is a **latent** drop that would occur only if the fold were removed **without** adding the carrier. A test-only "tolerance" guard would therefore assert behavior production does not have. The third task is consequently an **executable production-contract change** touching both seams **together**: preserve unknown top-level extension keys in place (stop relocating them under `docline` **and** add a `BaseFrontmatter` carrier so `ToMap` re-emits them at the top level instead of leaving the latent drop) — not a filesystem-containment guard. The size-extension regression is split into a RED-first integration task observed failing before the seam change.
- The base JSON schema itself also contradicts the open base contract: `schemas/docline/base-frontmatter-v1.schema.json` declares top-level `additionalProperties: false`, which **rejects** producer-owned top-level extension keys. Opening it is a separate width-isolated **schema task** (`107.011-T`) that makes the base schema **accept** producer-owned top-level extension properties generally — the base schema neither enumerates nor validates the size fields and encodes no declared-vs-legacy distinction; the base validator validates only the docline-owned base fields and intentionally ignores extension semantics, and validation of size semantics is owned by backlogit's future **derived** contract — with test-first schema-validation coverage. The authoring guide (`docs/docline-frontmatter-authoring-guide.md`), which currently says every non-contract key belongs under `docline` and that migration "moves every unknown key there", is corrected to the base/extension **inheritance model** by a separate **docs task** (`107.012-T`). The earlier "no schema changes" framing was therefore false at the shipment level and is corrected: the Go codec task (`107.009-T`) makes no schema change, but the shipment carries a bounded schema change in `107.011-T`.
...
3. `107.010-T`: **RED-first integration regression** (`tests/integration/docline_extension_compat_test.go`, test-only) proving representative backlogit-owned extension keys (`size`, `size_source`, `size_ruleset_version`) stay top-level, remain **semantically/deep-value equivalent** with unchanged document body bytes and idempotent normalization — **not** raw frontmatter byte/lexical preservation, since `Normalize`/`mdfront.Encode` reserializes YAML and may canonicalize ordering, quotes, scalar spelling, comments, and anchors (a raw-byte/YAML-node design is out of scope unless explicitly chosen later) — and are never validated/defaulted/synthesized by docline (docline preserves and re-emits them in place with semantic/deep-value equivalence but attaches no semantics); observed **failing at HEAD**, **sequenced after (dependent on) the `107.001-T` live-corpus RED**, and the dependency gate for `107.009-T`;
4. `107.009-T`: **production-contract change** to both docline seams — `internal/docline/normalize.go` stops folding non-contract top-level keys under the `docline` namespace, **and** `internal/docline/frontmatter.go` gains a `BaseFrontmatter` carrier so `FromMap` captures and `ToMap` re-emits unknown top-level keys instead of dropping them (`ToMap` otherwise emits only base-contract fields plus `docline`); the two contradictory normalization unit tests (`TestNormalize_ContractSurfaceHoldsOnlyContractKeys`, `TestNormalize_FoldsLegacyTypeUnderDocline`) in `normalize_test.go` are updated to expect pass-through, while `policy_test.go` `TestIsContractField` remains valid and unchanged. Docline validates/normalizes only its own base fields; it re-emits/serializes caller-provided extension properties at the top level when present but must never own, synthesize, default, validate, or semantically interpret extension semantics. This task implements **no** production filesystem containment and asserts **no** realpath/symlink guarantee.
5. `107.011-T`: **schema-contract change** opening `schemas/docline/base-frontmatter-v1.schema.json` so it **accepts** producer-owned top-level extension properties generally (for example `additionalProperties: true` or an equivalently open construct), **without** enumerating or validating the backlogit size fields and **without** any base-schema distinction between "declared/sanctioned" and "arbitrary legacy" extension keys, with **test-first** schema-validation coverage; the base validator continues to validate only the docline-owned base fields and intentionally ignores extension semantics. Gated behind the `107.010-T` RED and correlated with `107.009-T`. Validation of size semantics is owned by backlogit's future **derived** contract, not the base schema. This is the schema change the earlier no-schema-change framing omitted.
6. `107.012-T`: **documentation change** rewriting `docs/docline-frontmatter-authoring-guide.md` from the "move every non-contract key under `docline`" model to the base/extension **inheritance model** — docline is the base contract and owns and validates only base fields; docline **accepts** and **preserves** top-level extension properties in place — re-emitting/serializing caller-provided extension properties at the top level when present — but never interprets, defaults, validates, or synthesizes them; derived producers (backlogit) own top-level extension fields and their semantics are validated by backlogit's future **derived** contract; the guide explains extension acceptance/preservation vs producer-owned validation and forward-only legacy migration, with **no** base-layer distinction between "declared/sanctioned" and "arbitrary legacy" keys and without implying docline validates extension semantics; the canonical derived-key location is top-level. Depends on `107.009-T` and `107.011-T`.
...
Git determines tracked candidates and production `internal/docline.Scope()` filters them; this is the only include/exclude authority. The base docline contract is open/extensible: optional producer-specific keys — including the backlogit-owned `size`, `size_source`, and `size_ruleset_version` extensions — may be present without docline owning their meaning, and MUST be preserved unchanged at the top level. The extension milestone (`107.009-T`) changes both production seams — the normalization seam (`normalize.go`, stop folding under `docline`) and the serialization seam (`frontmatter.go`, `BaseFrontmatter` carries and re-emits unknown top-level keys) — so such optional keys pass through **in place** rather than being folded under the `docline` namespace or dropped by `ToMap`, and updates the two contradictory normalization unit tests; the size-extension regression is the RED-first `107.010-T`. Docline validates/normalizes only its own base fields; it re-emits/serializes caller-provided extension properties at the top level when present but never validates, defaults, rewrites, or synthesizes extension semantics. No realpath, symlink, junction, or reparse containment is implemented or claimed by any milestone; production containment stays lexical (`core.SafeResolve`) and is out of scope for this guard. Preservation and validation are intentionally **separate** responsibilities under the base/derived model: the codec (`107.009-T`) losslessly preserves all unknown top-level keys in place, and the base schema (`107.011-T`) **accepts** producer-owned top-level extension properties (open), but **neither validates their semantics** — the base schema does not enumerate or validate the size fields and encodes no "declared vs legacy" distinction, and validation of `size`/`size_source`/`size_ruleset_version` semantics is owned by backlogit's future **derived** contract. `107.012-T` documents this base/derived split (extension acceptance/preservation vs producer-owned validation) and the forward-only legacy migration. There is no missing selective codec migration and no base-schema sanctioning of specific size fields.

## Scope Boundary

The untracked `docs/decisions/2026-07-13-scratch-spike.md` is not read for mutation, edited, deleted, staged, or backfilled. The production changes in this guard are bounded to three concerns in three width-isolated tasks: the top-level pass-through of non-contract keys across the normalization seam (`normalize.go`) and the serialization seam (`frontmatter.go`) in `107.009-T`; the base JSON schema opening to accept additional producer-owned top-level extension properties (`schemas/docline/base-frontmatter-v1.schema.json`) in `107.011-T`; and the authoring-guide inheritance-model rewrite (`docs/docline-frontmatter-authoring-guide.md`) in `107.012-T`. Base-field validation semantics remain unchanged: the Go validator `internal/docline.Validate` continues to enforce the required fields, their minLength, the content_sha256 pattern, and the closed doc_type vocabulary (the base schema itself declares no doc_type enum; the closed vocabulary is a validator-only invariant, not a schema constraint). The ingested_at date-time format is the inverse case: it is a JSON-schema-only constraint that the hand-rolled Go validator does not enforce (ValidateFields checks required-field presence, minLength - rejecting whitespace-only values via strings.TrimSpace, which is stricter than JSON-Schema minLength:1 that accepts a single space - the doc_type vocabulary, and the content_sha256 pattern, but never parses ingested_at as RFC3339; the Go validator is thus a deliberate approximation of the schema, not a faithful mirror). `size` semantics (validation, defaults, provenance, composition) remain backlogit-owned and out of scope for this decision (they are the subject of the separate size spike, 096-S).
...
- **gofmt remediation scope boundary (gate runs; no-new-violations criterion).** *Principle/heuristic:* quality-gate honesty + width isolation — the authoritative `gofmt -l .` gate is run (never skipped), and this shipment is gated on introducing no new formatting violations (its changed Go files must be absent from `gofmt -l .` output) while unrelated pre-existing debt is scoped out and documented separately (a dedicated repo-wide remediation item is a recommended follow-up, not yet filed). *Justification:* the repository carries pre-existing `gofmt` debt (26 unrelated files at authoring time); requiring this width-isolated shipment to also remediate them would force unrelated cleanup and silently expand blast radius. *Rejected simpler alternative:* blocking this shipment until repo-wide `gofmt -l .` emits nothing (rejected because it couples this bounded guard to unrelated cleanup and violates width isolation; the pre-existing debt is instead documented here and scoped out, with a dedicated repo-wide remediation item recommended as a follow-up (not yet filed); it is not skipped or accepted, and this shipment's gate is no-new-violations rather than repo-wide-clean).
