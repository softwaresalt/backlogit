...
description: Complete unbounded golangci-lint v2.13.2 baseline inventory (489 findings across 160 files) and one-owner-per-finding mapping for the 175-F baseline-convergence DAG
doc_type: decision
schema_version: "1.0"
...
## Provenance
The authoritative baseline is the complete unbounded golangci-lint v2.13.2 structured run captured with the same invocation semantics as `scripts/verify-baseline-lint.ps1` (uncapped: `--max-issues-per-linter 0 --max-same-issues 0 --uniq-by-line=false`, JSON to stdout). The run yields exactly **489 findings across 160 files** (459 errcheck + 30 staticcheck). This complete count is authoritative; the default display cap truncates the first page and is never used to scope the shipment.
The committed machine-readable inventory lives at `docs/decisions/baseline-lint-inventory-489.json` (SHA-256 `7330d6baecc93e9cac4f4f96438a21565994106984f50c239cd1e6d8f86d2b66`). Each row carries the exact identity fields `path`, `line`, `column`, `linter`, `message`, `owner_task_id`. Identity tuples are unique; every finding has exactly one remediation owner.
## Reconciliation
By linter:
- `errcheck`: 459
- `staticcheck`: 30

By package (dir): 15 packages carry findings. Top packages:
- `internal/cli`: 234
- `internal/core`: 74
...
Total files with findings: 160.
## Ownership mapping (one owner per finding)
Exactly 90 finding-remediation tasks own the 489 findings. Each remediation task owns 1-16 exact findings across at most 2 files/packages. Baseline-control (U1 `175.001-T`), support (U40 `175.040-T`, U14 `175.014-T`), and terminal-convergence (U12 `175.012-T`) own zero findings.
...
Every identity, grouped by owner in ascending task order.
...
## Bounds verification
- Maximum findings owned by any single task: 16 (bound: 16).
- Maximum files/packages owned by any single task: 2 (bound: 2).
- Windows-only owned files (host-gated, U19-U22 style guard): 4 — `internal/core/shipment_reconcile_append_windows.go`, `internal/core/shipment_reconcile_evidence_windows.go`, `internal/core/shipment_reconcile_fs_windows.go`, `internal/core/shipment_reconcile_snapshot_windows.go`.
- Same-file slice groups (ordered, never co-wave): [`175.011-T`, `175.041-T`]; [`175.042-T`, `175.043-T`, `175.044-T`]; [`175.038-T`, `175.045-T`].
