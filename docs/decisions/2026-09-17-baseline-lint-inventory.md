...
The committed machine-readable inventory lives at `docs/decisions/baseline-lint-inventory.json` (SHA-256 of the canonical checked-in LF inventory bytes — equivalently the post-U1 LF working-tree bytes, not the host CRLF checkout — `bbb20803209fcf26af03e8dd13af1ad6236629fb2c7432caf14be5badddc5f4c`). It carries `primary_goos: windows`, `supported_goos: [windows, linux]`, the 489-row Windows `findings` array, the 8-row Linux-only `additional_findings` array (each tagged `goos: linux`), and `surface_exclusions` (`windows`: 0, `linux`: the 4 Windows-only keys). Under schema `baseline-lint-platform-inventory/v3` each row additionally carries the line-stable ownership fields `group_key`, `group_fingerprint`, `group_ordinal`, and `group_multiplicity`, and the inventory declares an `identity_model` (`line-stable-group-multiset/v1`) plus a `group_index`. Durable ownership identity is the stable group key `(path, linter, normalized_message)` fingerprinted as `sha256(json([path, linter, normalized_message]))` with per-task occurrence multiplicity; `line` and `column` are retained ONLY as diagnostic baseline coordinates (shown in the human-readable table below) and are never part of ownership identity. Every governed occurrence has exactly one remediation owner as an exact disjoint occurrence-count partition of its stable group.
## Reconciliation
