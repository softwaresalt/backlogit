...
## Standalone Plugin and Autoharness Adoption
...
harness. In that path, Autoharness composes the constitution, policies,
instructions, agents, and skills from templates and writes the tailored output
into `.github/`. It replaces the standalone plugin's agent and skill bundle,
not the backlogit runtime that generated registries can call through
`backlogit` and `backlogit mcp`.

The generated files in this repository are tracked in
[`.autoharness/harness-manifest.yaml`](../.autoharness/harness-manifest.yaml),
and many generated artifacts also carry `Generated by autoharness` footers.

That distinction matters for drift. `plugin/skills/` is the standalone product
bundle; `.github/skills/` is repo-local Autoharness material. The trees are not
interchangeable, and a workspace should avoid installing both surfaces for the
same harness. See [Installation](installation.md#which-installation-path) and
the [Plugin Guide](plugin-guide.md#skill-locations-and-drift) for the decision
point.
