...
No new *operational* follow-ups were generated by this closure (runtime
verification passed clean). Pre-existing RUN-1 follow-up stashes are deferred to
Stage triage (see Source Artifact Cleanup).

One **product-code follow-up** was surfaced during the closure review and is
deferred to Stage (Ship may not create stash artifacts):

- **ArchiveItem pre-archived stamping defect (systemic).** When `shipment ship`
  re-archives items that are already at the archive path (e.g. tasks archived at
  done-time, then formally archived at ship-time), `core.ArchiveItem` stamps
  `archived_from` with the file's *current* (archive) path, producing a
  self-reference instead of the original `.backlogit/queue/<id>.md` path. This
  contradicts the ArchiveItem contract (`internal/core/archive_test.go:70`) and
  breaks invertible unarchive/restore. The condition is **pre-existing and
  systemic** — present across ~130 prior archived records — so it is not unique
  to this shipment. This closure corrected the 12 in-diff 065 records
  (commit `166c5a53`) as backlog-state hygiene, but the production root cause
  (ArchiveItem should preserve/derive the canonical queue path when re-stamping
  an already-archived file) plus a one-time data migration for the remaining
  archive records should be triaged and fixed by Stage. Recommended stash:
  *"ArchiveItem: preserve canonical queue path in archived_from for
  pre-archived items + migrate legacy self-referential records."*
