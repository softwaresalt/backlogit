...
| `docs/compound/2026-07-13-copilot-review-loop-convergence.md` | create | New `doc_type: learning` entry: on rulesets where `copilot_code_review` auto-triggers on every push (with `required_review_thread_resolution: true`), each push spawns a fresh review that may raise new threads while prior unresolved threads persist — `dismiss_stale_reviews_on_push` dismisses stale *approvals*, not `COMMENTED` Copilot reviews or their threads. Batch fixes to minimize review passes and, per §1.8, use the cycle cap to stop automated *pushing* — **not** to clear the merge gate. Only fixed or objectively invalid/informational threads may be resolved; valid unresolved findings stay merge-blocking until fixed or explicitly operator-overridden (else halt and escalate). Framed honestly — the loop was NOT triggered in the 091-S feature PR (clean review, 0 threads); the rule is derived from the verifiable ruleset config + §1.8. |
