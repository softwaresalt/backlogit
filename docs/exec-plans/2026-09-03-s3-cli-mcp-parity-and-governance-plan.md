...
* Scope:
  1. `internal/mcp/docs_tools.go`: register `backlogit_docs_classify` wrapping `docline.Classify`, mirroring `backlogit_docs_lint` registration. Because `Classify` is pure, the handler MUST perform its OWN explicit workspace-containment pre-check on the agent-supplied `path` (`core.SafeResolve` against `s.RootPath`; reject traversal/absolute/escape) and map that failure to `ValidationFailed`, matching the sibling docs tools' `ErrPathEscapesWorkspace → validation_failed` behavior — do NOT claim `Classify` emits the sentinel.
...
* Acceptance: MCP `backlogit_docs_classify` returns the same `DocType` as CLI `backlogit docs classify <path>`; a containment-escaping `path` maps to `validation_failed`; the registry drift/parity test passes with the new row and the allow-list removal; the parity matrix reflects 57 tools.
