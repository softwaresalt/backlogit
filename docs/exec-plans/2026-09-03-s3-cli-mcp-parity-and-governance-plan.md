...
* Scope:
  1. Because `docline.Classify` is a PURE path-string classifier with NO containment, introduce a SHARED containment helper (e.g. `docline.ValidateClassifyPath(root, path)` using `core.SafeResolve`) and call it from BOTH surfaces before `Classify`, so traversal/absolute/escape inputs are rejected IDENTICALLY (no cross-surface asymmetry):
     - `internal/mcp/docs_tools.go`: register `backlogit_docs_classify` wrapping `docline.Classify`, mirroring `backlogit_docs_lint`; on a containment failure map to `ValidationFailed` (matching the sibling docs tools). Do NOT claim `Classify` emits `ErrPathEscapesWorkspace`.
...
* Acceptance: MCP `backlogit_docs_classify` returns the same `DocType` as CLI `backlogit docs classify <path>`; a traversal/absolute/escaping `path` is REJECTED IDENTICALLY on BOTH CLI and MCP (invalid-input parity test) — never classified on one surface and rejected on the other — mapping to `validation_failed` on MCP; the registry drift/parity test passes with the new row and the allow-list removal; the parity matrix reflects 57 tools.
