...
* Compatibility: purely additive — new in-package `MigrationPlan.Findings`; a new `findings` JSON key on `MigrateReport` that is always `[]` until U2c populates it; no existing field changes; existing consumers ignore the new key. `ApplyMigration` untouched.
...
* Acceptance (direct, reachable at U2b time — does NOT depend on U2c's report-and-continue, since pre-U2c `PlanMigration` still hard-aborts and never populates `Findings`):
  - Guard: constructing a `MigrationPlan` with a non-empty `Findings` and calling `ApplyMigration` returns `ErrPlanHasFindings` and performs ZERO writes; a plan with empty `Findings` applies normally.
  - Renderer: `writeMigrateResult` over a constructed plan carrying `Findings` renders them in BOTH text (mirroring `printLintText`) and JSON (a discrete top-level `findings` array), for dry-run and apply-rejection output alike.
  - MCP error-mapper: `errors.Is(err, ErrPlanHasFindings)` maps to a structured `IsError:true` result with a DISTINCT error-type string `plan_has_findings` (NOT the generic `validation_failed`, so an agent can disambiguate a corpus-content rejection from a `--path` rejection) and a discrete `findings` array field via a dedicated response struct (NOT flattened into `message`; the 2-field `ValidationFailed(detail)` helper is insufficient). Assert error-type != `internal` and `IsError:true` (guards against sentinel fall-through to `InternalError`).
  - CLI parity: the CLI apply branch, on `errors.Is(err, ErrPlanHasFindings)`, renders the migrate report via `writeMigrateResult` (text+JSON, including `plan.Findings`) BEFORE returning a non-zero sentinel (mirroring the existing `errLintViolations` render-then-signal pattern), so CLI apply JSON contains the findings array at MCP parity.
  (The END-TO-END mixed-corpus test — a real malformed file flowing through `PlanMigration` → findings → apply guard — belongs to U2c, which is what makes `PlanMigration` report-and-continue.)
...
* Agent-Native Parity Reviewer (triggered by U2b's MCP tool error/result contract change) — PASS. P2 parity-precision items folded into U2b acceptance: discrete `findings` array field via a dedicated response struct, distinct `plan_has_findings` error-type, CLI render-before-return.
* Security Lens Reviewer (triggered by the API/tool-surface change) — security-PASS. Write-path integrity preserved (guard sits above preflight/TOCTOU; containment stays fatal); two P3 data-exposure/trust-boundary advisories folded into U2b/U2c acceptance (IsError+zero-write assertion; `Finding.Fix` carries only the frontmatter parser message, not raw body bytes).

The code-level Security Reviewer was not separately risk-triggered at plan stage; the Security Lens Reviewer covered the API-surface security review (in-package docline refactor; no auth/secrets/injection/network surface added).
