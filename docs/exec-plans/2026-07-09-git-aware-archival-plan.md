...
does not intentionally record a Git rename. The archive and restore lifecycle
should use `git mv` only when the workspace is inside a Git repository and the
specific artifact path is tracked, then fail closed if `git mv` itself fails.

## Requirements Trace

| Requirement | Implementation action |
|---|---|
| Detect git repository and per-file tracked state | Add a small helper around `git rev-parse --show-toplevel` and `git ls-files --error-unmatch -- <path>` with Windows-safe paths |
| Tracked artifact in git repo uses `git mv` for archive | Replace the final tracked-file move step in `ArchiveItem` with `git mv` after archive frontmatter is prepared, preserving existing destination collision checks |
...
| Fail closed on unexpected `git mv` errors | Return contextual errors instead of silently falling back after a selected `git mv` strategy fails |
| Decide staged-vs-unstaged behavior | Document that `git mv` stages the rename in Git, while content/frontmatter edits remain normal working-tree changes; tests assert the observable status |
...
* Tests: table-driven unit tests for non-git workspace, git unavailable, git
  repo with untracked file, and git repo with tracked file
* Posture: test-first

### T2 - Git-aware archival path

* Changes: use the helper inside `ArchiveItem` after destination safety checks
  and before DB synchronization
* Files: `internal/core/archive.go`, archive tests
* Tests: integration-style temp git repo test asserting `git log --follow`
  retains history for archived tracked artifacts; regression test for non-git
  filesystem moves
...
* Changes: apply the same move strategy in `UnarchiveItem`, including the
  canonical restore path and clobber-refusal guard
...
* Tests: force a selected `git mv` failure and assert the error includes move
  context without falling back to filesystem rename
* Posture: test-first
...
* Keep `git mv` staged behavior. `git mv` intentionally updates the index; this
  matches the user's explicit requirement to preserve rename history for tracked
  backlog artifacts and leaves commit discipline to the surrounding workflow.
...
* The current archive path writes modified frontmatter before removing the
  source. Ship should preserve the existing rollback behavior around DB update
  failures while replacing the physical move step.
* `git log --follow` assertions require a real temp Git repository. Tests should
  skip only when `git` is genuinely unavailable, not when backlogit behavior is
  incorrect.
