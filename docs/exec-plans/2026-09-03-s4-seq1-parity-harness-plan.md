...
driver) is independent; U2 depends on U1 + U4a-declarations (it emits the parity
evidence artifact); U3 depends on U2; U4b depends on U4a-behavior (it uses
`Validate`/`DecodeAndValidate`). Order: U4a-declarations and U1 in parallel →
U4a-behavior → U2 → U3; U4b after U4a-behavior. Backlog mapping: `156.001-T` (U1),
