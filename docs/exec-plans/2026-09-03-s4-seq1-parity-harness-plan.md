...
* Scope: compare the three captures DIMENSION-BY-DIMENSION using APPLICABILITY / STATUS markers, NOT literal cross-surface equality. Each dimension carries a per-surface applicability marker (`applicable` / `not_applicable`) and a status; the comparator compares a dimension across surfaces ONLY where it is applicable on each:
  ...
  - durable side effects / post-state (Markdown + SQLite + event stream) → applicable to ALL MUTATING surfaces: capture each surface's ISOLATED post-state (on its own cloned root) and compare them THROUGH THE DETERMINISTIC PROJECTION (volatile wall-clock fields `created_at`/`updated_at`/event-timestamps normalized or stripped per U1), so a CLI or MCP path that returns the right response but fails to persist the expected Markdown/DB/event change is detected while timestamp noise does NOT false-diverge (durable side effects are a cross-surface contract per the Problem Frame, NOT internal-only); `not_applicable` only for genuinely non-persisting (read-only) scenarios;
...
* Acceptance: each seeded scenario is present and GREEN against current code; a scenario that encodes a KNOWN cross-surface drift (e.g. the CLI `--json` gate payload omitting `remediation`/`retry_after_ms`) is green by asserting the EXPECTED divergence AND naming its concrete tracked defect (the standalone feature `166-F`, OUTSIDE the shipment hierarchy, report-only), NOT by being left red; any genuinely NEW/undocumented red fails closed and must be triaged to an existing tracked defect (never left as an undocumented failing test). Depends on U2.
...
* Scope: `EvidenceArtifact` is the SHARED, FAMILY-AGNOSTIC ENVELOPE consumed by ALL of S4-S11 — `EvidenceArtifact{ SchemaVersion int `json:"schema_version"`; ProducingTask string `json:"producing_task"`; ProducingCommit string `json:"producing_commit"`; NodeFamily string `json:"node_family"`; Applicability []string `json:"applicability"`; ValidatorIdentity string `json:"validator_identity"`; VerifiedEvidence json.RawMessage `json:"verified_evidence"`; Status string `json:"status"` }` (ALL OUTER JSON tags snake_case and explicit). The `VerifiedEvidence` payload is a DISCRIMINATED, node-family-specific typed document — NOT one parity-shaped struct forced on every producer (S5 records representation convergence, S9 records separate observed red/green results; forcing those into parity fields or the `Detail` string would defeat the versioned schema). A `FamilyPayload` interface (`CanonicalMap() map[string]any` + `Validate() error`) plus a VERSION-AWARE family REGISTRY keyed by the COMPOSITE `(SchemaVersion, NodeFamily)` — `map[int]map[string]func() FamilyPayload` — decodes `VerifiedEvidence` into the CONCRETE family struct for THAT schema version. Version-awareness is required so that when later families or revised payload shapes register, decode/`Validate` select the allowed family SET and the concrete payload SHAPE from the artifact's own `SchemaVersion` (a flat versionless `map[string]factory` could not tell which family/shape belongs to which version, and the `{N, N-1}` window needs per-version payload shapes). At VERSION 1 the ONLY registered family is `parity` (S4): `ParityEvidence{ ScenarioID string `json:"scenario_id"`; Dimension string `json:"dimension"`; Surfaces []string `json:"surfaces"`; DivergentFields []string `json:"divergent_fields"`; ExpectedDivergence bool `json:"expected_divergence"`; TrackedDefect string `json:"tracked_defect"`; Detail string `json:"detail"` }` (snake_case tags; `Surfaces`/`DivergentFields` non-nil empty slices per the always-array contract; `TrackedDefect` empty-string-valid only when `ExpectedDivergence` is false). Later producers REGISTER their OWN typed family payload (S5 `RepresentationEvidence`, S9 `RedGreenEvidence`, …) each with its OWN per-family golden — they do NOT overload the parity payload or hide data in `Detail`. Because BOTH the envelope AND the registered family payload participate in the golden fixture and `Canonical()` ordering, an envelope OR payload-shape change IS a schema change; with:
  ...
  - NODE-FAMILY ALLOWED SET (enumerated, not vague; per-version): the allowed `NodeFamily` values for a given artifact are exactly `knownFamilies(a.SchemaVersion)`. At version 1 the registry entry for version 1 contains exactly one exported constant `NodeFamilyParity = "parity"` (S4). Each later producer shipment registers its own exported constant (e.g. `NodeFamilyRepresentation`, `NodeFamilyRedGreen`) FOR the schema version(s) it targets, WITH its typed payload and golden; adding a family is a documented schema evolution. `Validate` rejects a `NodeFamily` not in `knownFamilies(a.SchemaVersion)`;
  - VALIDATION INVARIANTS enforced by `Validate` (required-field + allowed-value rules — because absent JSON fields decode to ZERO VALUES even under `DisallowUnknownFields`, a producer emitting a zero-valued-but-shape-valid artifact MUST be rejected): `SchemaVersion` ∈ `knownVersions`; `ProducingTask`, `ProducingCommit`, `ValidatorIdentity` non-empty; `NodeFamily` ∈ `knownFamilies(SchemaVersion)`; `Applicability` non-empty with each element ∈ `{cli, mcp, internal}`; `Status` ∈ `{pass, report_only, fail}`; PLUS the family payload's OWN `Validate()` (for `ParityEvidence` at v1: `ScenarioID` and `Dimension` non-empty; `Surfaces` non-empty with each ∈ `{cli, mcp, internal}`; when `ExpectedDivergence` is true then `TrackedDefect` MUST be a non-empty concrete backlog ID; when `Status != pass` then `DivergentFields` non-empty). `DecodeAndValidate` runs BOTH envelope and family `Validate` after shape decode (returning `ErrValidation` on failure), so no schema-valid-but-invariant-violating artifact is accepted;
  ...
  - UNKNOWN-VERSION rejection AND MALFORMED-WIRE rejection on decode — a two-phase decode peeks `schema_version` before full unmarshal; a version outside `knownVersions` returns a TYPED error; decode uses `DisallowUnknownFields` so an unstamped additive drift is REJECTED. Because `encoding/json` alone still applies LAST-VALUE-WINS on duplicate members and CASE-INSENSITIVE field matching (so `DisallowUnknownFields` does NOT catch a duplicated key or a case-folded alias like `Schema_Version`/`SCHEMA_VERSION`), `DecodeAndValidate` FIRST runs a RAW `json.Decoder` TOKEN-STREAM pre-scan over BOTH the envelope object AND the nested `verified_evidence` object that rejects (a) any duplicate object member (case-sensitively equal OR case-fold-equal to a prior member) and (b) any key whose casing does not EXACTLY match a declared struct tag — before `json.Unmarshal` runs. Conformance cases cover duplicate `schema_version`, a case-variant alias, and a duplicate nested payload key;
...
* Acceptance: the SHARED ENVELOPE + the family REGISTRY + the version-1 `parity` family payload are defined and versioned; `Canonical()` (wrapping `internal/canonical`, merging the family `CanonicalMap()`) is byte-stable, asserted against a CHECKED-IN raw-bytes GOLDEN fixture PER FAMILY (so any envelope/payload field/shape/tag change breaks the golden — avoiding the round-trip `DeepEqual` false-green of `2026-07-21-omitempty-defeats-arrays`); empty collections are asserted present-and-array on the raw JSON (`.([]any)`), NOT via struct↔canonical `DeepEqual`; an unknown/unstamped-additive version, a `NodeFamily` not in `knownFamilies()`, a DUPLICATE key, and a CASE-VARIANT alias each fail closed with a typed error; the S4 harness (U2) emits a `NodeFamilyParity` artifact against it. Independent foundation unit. Single responsibility (the versioned contract package).
* Harness-architect decomposition (declaration-first per `.github/skills/harness-architect/SKILL.md`): because the new exported declarations carry behavior, this splits into TWO backlog tasks — `156.004-T` = the DECLARATIONS + source-shape harness (the `EvidenceArtifact` envelope, the `FamilyPayload` interface, the family registry + `NodeFamilyParity` constant, the `ParityEvidence` v1 payload, API signatures, red shape harness), and `156.006-T` = the BEHAVIOR (`canonicalMap`/`Canonical` wrapper merging the family map, two-phase version peek, raw token-stream duplicate-key + case-exact pre-scan, `DisallowUnknownFields`, envelope+family `Validate`, per-family golden fixture, conformance). U4b's `Classify`/`Validate`/`DecodeAndValidate` consumers depend on `156.006-T`.
...
* Scope: WITHOUT implementing any later shipment, encode the FUTURE producer AND consumer obligations symmetrically:
  ...
  - The shared decode + classification API is PRODUCTION code in `internal/faultline` (NO `testing` import — S10/S11 import it): `DecodeAndValidate([]byte) (EvidenceArtifact, FamilyPayload, error)` enforces the token-stream duplicate/case pre-scan + `DisallowUnknownFields` + trailing-data (`io.EOF`) check + envelope/family version/shape validation, plus `Validate(EvidenceArtifact, FamilyPayload) error` for post-construction checks.
  - FOUR-OUTCOME CLASSIFICATION OWNER (report-only behavior owned HERE, in the foundation): a production `func Classify(present bool, raw []byte) (Outcome, EvidenceArtifact, FamilyPayload, error)` with `type Outcome int` and constants `OutcomeAbsent`, `OutcomeValid`, `OutcomeUnknownOrMalformed`, `OutcomeValidationFailed`. `Classify` maps: (1) `present == false` → `OutcomeAbsent` (benign, silent); (2) `present`, `DecodeAndValidate` succeeds → `OutcomeValid` (blocking once promoted); (3) `present` but `DecodeAndValidate` returns `ErrMalformed`/`ErrUnknownVersion`/`ErrUnknownFamily` → `OutcomeUnknownOrMalformed` (LOUD); (4) `present`, but `DecodeAndValidate` returns `ErrValidation` → `OutcomeValidationFailed` (LOUD). The distinction between (3) and (4) is DETERMINISTIC because `DecodeAndValidate` returns TYPED error categories (not one undifferentiated error), which `Classify` discriminates via `errors.Is`. The FOUR-outcome test lives HERE in U4b against `Classify` (NO S10/S11 code is written). S10/S11 consume `Classify` on their accept path.
