...
**Schema and authoring-guide correction (PR #241).** The base JSON schema itself also contradicts the open base contract: `schemas/docline/base-frontmatter-v1.schema.json` declares top-level `additionalProperties: false`, which **rejects** producer-owned top-level extension keys, and the authoring guide (`docs/docline-frontmatter-authoring-guide.md`) still says every non-contract key belongs under `docline` (migration "moves every unknown key there"). These are corrected by two additional width-isolated tasks: `107.011-T` opens the base schema for declared producer-owned top-level extension fields (least-permissive compatible mechanism, test-first schema-validation coverage) and `107.012-T` rewrites the guide to the base/extension inheritance model. The earlier "no schema changes" framing was false at the shipment level and is corrected here: `107.009-T` (the Go codec task) makes no schema change, but this shipment **does** carry a bounded JSON-schema change in `107.011-T`.
...
| D4a | Observe extension RED before the seam change | `107.010-T` (`tests/integration/docline_extension_compat_test.go`) fails at current HEAD (keys dropped) and is the dependency gate for `107.009-T`. It also inventories tracked docs for already-folded producer keys (`docline.size*`) and records the forward-only zero-match evidence. |
| D4b | Open the base schema for declared producer extensions | `107.011-T` opens `schemas/docline/base-frontmatter-v1.schema.json` (currently top-level `additionalProperties: false`) for declared producer-owned top-level extension fields using the least-permissive compatible mechanism, with test-first schema-validation coverage. Gated behind the `107.010-T` RED; distinguishes declared producer extensions from legacy arbitrary keys. |
...
### `107.011-T` — Open the base JSON schema for declared producer extensions (schema, test-first)

**File:** `schemas/docline/base-frontmatter-v1.schema.json` (plus a schema-validation test).

The base schema currently declares top-level `additionalProperties: false`, which **contradicts** the open base contract and rejects producer-owned top-level extension keys (`size`, `size_source`, `size_ruleset_version`). Open it for declared producer-owned top-level extension fields using the **least-permissive compatible mechanism** justified by current schema/test patterns — for example declaring the sanctioned producer extension properties as optional typed top-level fields, or a scoped `patternProperties`/`$defs` namespace — rather than a blanket `additionalProperties: true`, so **declared** producer extensions validate at the top level while truly-unknown/legacy arbitrary keys stay distinguishable from sanctioned extensions.

**Test-first:** write the failing schema-validation coverage first (declared producer extension keys validate top-level; the chosen mechanism still distinguishes sanctioned extensions from legacy arbitrary keys), then change the schema to turn it green. The hand-rolled Go validator (`internal/docline/validate.go`) does not enforce `additionalProperties` today, so no Go behavior regression is introduced; it stays consistent with the schema contract. Width-isolated to the schema family; gated behind the `107.010-T` RED and correlated with `107.009-T` and `107.012-T` (the Schema-CLI-Docs Coupling Reviewer confirms the three stay consistent). No CLI-distribution change.

### `107.012-T` — Document the base/extension inheritance model (docs)

**File:** `docs/docline-frontmatter-authoring-guide.md`.

The guide currently states every non-contract key belongs under `docline` and that migration "moves every unknown key there (move, never drop)", contradicting the open base/extension contract. Rewrite the ownership model as an **inheritance model**: docline owns and validates only the base fields; derived producers such as backlogit **own** top-level extension fields (`size`, `size_source`, `size_ruleset_version`); docline **preserves** top-level extensions in place but never interprets, defaults, aggregates, validates, or emits them. Explicitly **distinguish** declared producer extensions (sanctioned top-level keys per the opened schema `107.011-T`) from legacy arbitrary keys, and describe the normalization/migration behavior for each. Name the canonical derived-key location explicitly: **top-level**. Body-preserving and docline-lint-clean; depends on `107.009-T` and `107.011-T` because the guide must describe the final codec + schema behavior. No production code or schema change in this task.
...
7. Run `go run ./cmd/backlogit docs lint` in a clean checkout, and run the schema-validation test added by `107.011-T` (declared producer extensions valid top-level; legacy arbitrary keys distinguishable).
...
| Schema-CLI-Docs Coupling Reviewer | triggered (the shipment now opens the base JSON schema and rewrites the authoring guide alongside the codec change) | The three correlated corrections are consistent: `107.009-T` (codec) preserves unknown top-level keys in place; `107.011-T` (schema) opens the base schema for **declared** producer extensions using the least-permissive compatible mechanism, keeping declared producer extensions distinguishable from legacy arbitrary keys; `107.012-T` (docs) documents that inheritance model and names top-level as the canonical location. Sequencing enforces coherence: `107.011-T` depends on the `107.010-T` RED; `107.012-T` depends on `107.009-T` + `107.011-T` so the guide reflects final behavior. Codec–schema coupling clarified (intentional separation, not a defect): the codec (`107.009-T`) losslessly **preserves all** unknown top-level keys in place while the schema (`107.011-T`) sanctions only **declared** producer extensions; preserved-but-undeclared legacy keys stay losslessly preserved but are **not** thereby schema-valid. There is **no missing selective codec migration** — preservation (codec) and validation (schema) are intentionally separate responsibilities; `107.012-T` documents this distinction and the lint/migration treatment for preserved-but-undeclared legacy keys (preserved in place, never silently relocated under `docline` or dropped, and MAY be surfaced by lint as undeclared/unsanctioned). Recorded as a P2 advisory (not blocking); the coupling reviewer gate re-runs at execution. No P0/P1. |

**Findings disposition:** P0 = 0, P1 = 0, P2 = 2 (advisory, dispositioned), P3 = advisory. **Prior latent P1 (serialization seam)** — the plan scoping the production change to `normalize.go` plus tests while `Normalize` serializes through `BaseFrontmatter.ToMap()` (which drops unknown top-level keys) — remains **resolved**: `107.009-T` explicitly includes `internal/docline/frontmatter.go` (carrier + `FromMap`/`ToMap` re-emission), the size regression is split into the RED-first `107.010-T`, and `policy_test.go` stays unchanged. **P1 raised this re-run (schema/authoring-guide contract mismatch) — resolved:** the base schema declared top-level `additionalProperties: false` (rejecting producer extensions) and the plan falsely claimed "no schema changes"; this is corrected by opening the schema in the new width-isolated `107.011-T` (test-first, least-permissive mechanism) and rewriting the guide in `107.012-T`, and by correcting the shipment-level claim. **P2 (downstream-consumer behavior change):** relocating fewer keys under `docline` is observable to any consumer reading `docline.<key>`; dispositioned — bounded to `internal/docline`, idempotent, body-preserving, and evidence shows **zero** tracked `docline.size*` keys and **no** Go consumer of `size_source`/`size_ruleset_version`, so no consumer contract breaks. **P2 (codec-vs-schema coupling):** the codec preserves all unknown top-level keys while the schema sanctions only declared producer extensions — this is an **intentional separation of responsibilities, not a missing selective migration**: preserved-but-undeclared legacy keys stay losslessly preserved but are not thereby schema-valid; dispositioned to the Schema-CLI-Docs Coupling Reviewer gate and to `107.012-T`, which documents the distinction and the lint/migration treatment for preserved-but-undeclared legacy keys. No finding blocks harvest.

**Plan hardening:** RE-EVALUATED for this re-run. The shipment now touches a JSON schema (`107.011-T`), which is an elevated-blast-radius signal, so plan-harden was assessed explicitly rather than skipped. The schema change is bounded: it opens **one** base-frontmatter schema for optional declared producer extension fields using the least-permissive compatible mechanism, is **test-first** (failing schema-validation coverage precedes the schema edit), touches no CLI distribution and no multiple template families, and performs no destructive operation. Mitigations: the Schema-CLI-Docs Coupling Reviewer gate, width isolation (schema-only in `107.011-T`, docs-only in `107.012-T`, Go-only in `107.009-T`), the `107.010-T` RED gate, and idempotency/body-preservation invariants. Given the bounded single-schema scope and these mitigations, a separate blocking `plan-harden` pass is not mandated; the elevated signal is recorded honestly and dispositioned in-plan. The codec change likewise stays bounded to `internal/docline` (`normalize.go` + `frontmatter.go`).
