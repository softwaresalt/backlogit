...
* **Injection Risk Assessment** — SQL injection, XSS, command injection, template injection, LDAP injection patterns in `cmd/**/*.go, internal/**/*.go`
...
Language-specific detection uses `password|secret|api_key|token|credential|private_key`.
...
1. Scan source files matching `cmd/**/*.go, internal/**/*.go` for unvalidated external inputs
2. Apply injection detection patterns from `password|secret|api_key|token|credential|private_key`
...
Score the workspace against OWASP Top 10 categories using `password|secret|api_key|token|credential|private_key`:
